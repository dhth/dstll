...
func (m Model) View() string {
	...
	for i, f := range m.files {
		...
		if m.selected == i {
			...
			if disabled {
				s.WriteString(m.Styles.DisabledSelected.Render(trim(m.cursor+selected, m.Width)))
			} else {
				s.WriteString(m.Styles.Selected.Render(trim(m.cursor+selected, m.Width)))
			}
			s.WriteRune('\n')
			continue
		}
		...
		s.WriteString(style.Render(trim("  "+fileName, m.Width)))
		s.WriteRune('\n')
	}
